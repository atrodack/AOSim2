%%Script to test out hybrid reconstructor approach
%PMH 20100225


clear;
numframes=30;     %number of frames to generate
WFS_FPS=1000;       %running at  "WFS_FPS" Hz
AO_STARTTIME=0.002; %Starting time of AO correction
PHASE_START=0.008;  %Starting time of phase correction
...
for i=1:numtrials  %Run complete program multiple times
    ...
    if(REBUILD) 
        ...
        RECONseg = AOReconstructor(A,DM,WFS);
        RECON = AOReconstructor(A,DM,WFS);
        %Set Scutoff high to make the cutoff be whatever the modes explored is.
        RECONseg.Scutoff=30000;
        RECON.Scutoff=30000;
        %ORDER = sqrt(NMODES*2);
        FULLORDER=0;
        SEGORDER=1;
        %Build a reconstructor for only the segment aberrations
        %RECONseg.sprogram(D,FULLORDER, d,SEGORDER,CENTERS);
        %Now build a reconstructor for the full aperture
        ORDER=3;
        show=true;
        ...
        %save GMTAO_Segmodes A DM WFS RECONseg d D % 
        save GMTAO_Demomodes A DM WFS RECON d D % 
        F = AOField(A);
        F.lambda = RECONseg.lambda;
        [x,y] = coords(F);
    else  %Load in pre-made reconstructors
        load GMTAO_dh1246
        %if(i==1)
            %load GMTAO_dh924
            %load GMTAO_Segmodesdh1155 
            %load GMTAO_Fullmodesdh57 
       ...
       % elseif (i==2)
        %    load GMTAO_dh1246
            ...
            %load GMTAO_Segmodesdh1155
            %load GMTAO_Fullmodesdh57
            %load GMTAO_Segmodes152 
            %load GMTAO_Fullmodes14 
            %load GMTAO548; 
            %load GMTAOs322;
        %else   
        %    load GMTAO_dh1616
