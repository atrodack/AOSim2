...
% 20090504 JLCodona: Extended this demo to simulate Josh Eisner's
% non-redundant masks.
...
load data/EisnerMasks

MASK_NUMBER = 4;

MASK = AOAperture;
MASK.name = sprintf('Eisner Mask %d',MASK_NUMBER);

S = AOSegment;
P = [0 0 EISNER_MASKS{MASK_NUMBER}.diam 1 MASK.dx 0 0 0 0];
S.pupils = P;
S.Offset = [0 0];
S.touch.make;

for n=1:size(EISNER_MASKS{MASK_NUMBER}.centers,1)
	MASK.addSegment(S,EISNER_MASKS{MASK_NUMBER}.centers(n,:));
end

% S = AOSegment;
% S.pupils = EISNER_MASKS{MASK_NUMBER}.pupils;
% % S.pupils = EISNER_MASKS{MASK_NUMBER}.pupils(1,:);
% S.touch.make;
% 
% MASK.addSegment(S);
...
for n=1:2000
	...
	subplot(N1,N2,5);
	OTF = fftshift(fft2(ifftshift2d(PSF)));
	plotCAmpl(OTF,1/6);
	axis xy;
	drawnow;
