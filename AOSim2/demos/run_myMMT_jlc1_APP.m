...
% close;
% clear classes;data/MMTAO_Model_JLC_20090426.mat
% 
% % Load in my pre-built GMT model.
% load data/MMTAO_Model_JLC_20090426.mat
...
% F.FFTSize = 2*size(A); % twice Nyquist
F.FFTSize = 3*size(A); % thrice Nyquist
...
!rm /tmp/FRAMES*
HISTS = [];
SLOPES = zeros(WFS.nSubAps*2,1000);
CUBE = [];
EXPOSURE = 1/60;
WFS_FPS = 523;
EXP_FRAMES = round(WFS_FPS*EXPOSURE);
...
[rv,s]=unix('whoami');
HEADER.USER = s(1:end-1);

[rv,s]=unix('hostname');
HEADER.HOST = s(1:end-1);

[rv,s]=unix('date');
HEADER.DATE = s(1:end-1);

HEADER.PUPIL = A.name;
HEADER.RECON = RECON.TrainingMethod;
HEADER.AO_Gain = gain;
% HEADER.OWDCD = OWDCD;
% HEADER.STREHL_CUTOFF = STREHL_CUTOFF;
% HEADER.DSCROSS_CUTOFF = DSCROSS_CUTOFF;
% HEADER.PROGRAM = 'doProj_20081229.m';
% HEADER.PREFIX = PREFIX;
% HEADER.SV_CUTOFF = SV_CUTOFF;

HEADER.WFS_FPS = WFS_FPS;
HEADER.EXPOSE = EXPOSURE;
HEADER.EXPFRAMS = EXP_FRAMES;
...
if(mod(n,EXP_FRAMES)==0)
	...
	fits_write_image('CCD_CUBE.fits',CUBE,HEADER);
	
    subplot(N1,N2,5);
    imagesc(log10(normalize(CCD)),[-5 0]);
    axis xy;
end
