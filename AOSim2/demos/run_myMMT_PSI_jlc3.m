...
DM2 = AODM(DM);
DM2.name = 'MMT DM336 Clone';
DM2.actuators = DM.actuators;
DM2.bconds = DM.bconds;

DM2.plotRegions; daspect([1 1 1]); drawnow;
...
TIMES = demean(0:1/WFS_FPS:16*EXPOSURE);
...
% SLOPES = zeros(WFS.nSubAps*2,length(TIMES));
...
%     TIMES = demean((1:NT)/WFS_FPS);
    ...
    [ALL_SLOPES,MASK] = all_slopes(WFS);
    SLOPES = zeros(length(ALL_SLOPES),length(TIMES));    
    HCUBE = zeros([size(HALO) length(TIMES)]);
    HCUBE0 = zeros([size(HALO) length(TIMES)]);
    ...
    GOP = GOPdate;
    save(sprintf('/data/T1/PSI/MattSim/NCP_%s_Look%03d.mat',GOP,nLook),...
        'GOP','HCUBE','HCUBE0','FOV','PLATE_SCALE','SCIENCE_WAVELENGTH',...
