...
GAMMA = 1;  % This is the gamma correction for the PSF image.
SCIENCE_WAVELENGTH = AOField.HBAND;

FOV_START = 0.2;
FOV_AO_ON = 0.2;

ZOOM_STARTTIME = 0.0;
ZOOM_ENDTIME = 0.00001;
...
if(REMAKE_ATMO)
    ...
    % WFlow = AOScreen(1024,0.22,500e-9);
...
% 		% This is NEW!
    ...
    ncube = mod(n-1,Ncube)+1;
    CUBE(:,:,ncube) = PSF;
    
    if(ncube==Ncube) % Dump the cube to disk...
        HEADER = struct;
        HEADER.time = t;
        HEADER.ATMOtime = ATMO.time;
        HEADER.n = n;
        nblock = ceil(n/Ncube);
        fprintf('Dumping PSF block %d, n=%d, t=%g\n',nblock,n,t);
        fits_write_image(sprintf('/tmp/AOSim2_PSFS_%04d.fits',nblock),CUBE,HEADER);
    end
	...
	axis xy;
    daspect([1 1 1]);
    title(sprintf('PSF (\\lambda=%.2g microns, \\gamma=%g) t=%.5f',...
